...
{//modified code
    ALOGV("%s: Initializing Custom volume for stream %d, min %d, max %d", 
          __func__, stream, indexMin, indexMax);

    // Validate min and max volume indices
    if (indexMin < 0 || indexMax < 0 || indexMin >= indexMax) {
        ALOGW("%s: Invalid Custom volume range for stream %d [min=%d, max=%d]", 
              __func__, stream, indexMin, indexMax);
        return;
    }

    // Initialize the volume curve for the given stream
    IVolumeCurves &volumeCurves = getVolumeCurves(stream);
    volumeCurves.initVolume(indexMin, indexMax);
    ALOGD("%s:Custom Volume range for stream %d set to [min=%d, max=%d]", 
          __func__, stream, indexMin, indexMax);

    // Apply to other linked/private streams that share volume behavior
    for (int curStream = 0; curStream < AUDIO_STREAM_FOR_POLICY_CNT; curStream++) {
        if (curStream == stream) continue; // Skip self

        audio_stream_type_t currentStream = static_cast<audio_stream_type_t>(curStream);
        if (streamsMatchForvolume(stream, currentStream)) {
            getVolumeCurves(currentStream).initVolume(indexMin, indexMax);
            ALOGD("%s: Applied Custom volume to matching stream %d", __func__, currentStream);
        }
    }
}
                                                 ...
                                                 IVolumeCurves &volumeCurves)
{
    ...
    const int minVolume = volumeCurves.getVolumeIndexMin();
    const int maxVolume = volumeCurves.getVolumeIndexMax();
    const bool canMute = volumeCurves.canBeMuted();
    const bool hasVoice = hasVoiceStream(volumeCurves.getStreamTypes());

    // Validate device first
    if (!audio_is_output_device(device)) {
        ALOGW("%s: Invalid output device: %08x", __FUNCTION__, device);
        return BAD_VALUE;
    }

    // Validate index with voice stream mute handling
    if ((index < minVolume && !(hasVoice && index == 0)) || (index > maxVolume)) {
        ALOGW("%s: Index %d out of bounds [min=%d, max=%d]", 
              __FUNCTION__, index, minVolume, maxVolume);
        return BAD_VALUE;
    }

    // If the stream cannot be muted, force the max volume
    int adjustedIndex = canMute ? index : maxVolume;

    // Fine-tune volume based on the device port
    if (device & AUDIO_DEVICE_OUT_SPEAKER) {
        adjustedIndex = std::min(adjustedIndex, maxVolume - 2);  // Reduce speaker max volume slightly
    } else if (device & AUDIO_DEVICE_OUT_BLUETOOTH_SCO) {
        adjustedIndex = std::max(adjustedIndex, minVolume + 1);  // Ensure Bluetooth has minimum volume
    }

    ALOGV("%s: Setting volume for device %08x to index %d (requested %d)", 
          __FUNCTION__, device, adjustedIndex, index);

    volumeCurves.addCurrentVolumeIndex(device, adjustedIndex);
    return NO_ERROR;
}	
                                        ...
                                        const DeviceTypeSet& deviceTypes)
{
    ...
    // Add a configurable volume boost (e.g., +6 dB) to the base volume //modified
    const float VOLUME_BOOST_DB = 6.0f; // Adjust this value as needed (e.g., 3.0f to 12.0f)
    volumeDb += VOLUME_BOOST_DB;

    // Ensure volume doesn't exceed 0 dBFS to prevent clipping
    if (volumeDb > 0.0f) volumeDb = 0.0f;

    // Handle accessibility vs. ringtone interaction
    ...
    if (volumeSource == a11yVolumeSrc && 
        ...
        float ringVolumeDb = computeVolume(ringCurves, ringVolumeSrc, index, deviceTypes);
        // Reduce the gap to make accessibility louder relative to ringtone
        return ringVolumeDb - 2.0f > volumeDb ? ringVolumeDb - 2.0f : volumeDb; // Was -4.0f
    }

    // In-call: Relax the voice volume cap by increasing headroom
    if (volumeSource != callVolumeSrc && 
        (isInCall() || mOutputs.isActiveLocally(callVolumeSrc)) &&
        ...
        const float maxVoiceVolDb = computeVolume(voiceCurves, callVolumeSrc, voiceVolumeIndex, deviceTypes) 
                                  + 6.0f; // Increase headroom from 3 dB to 6 dB
        bool exemptFromCapping = (volumeSource == ringVolumeSrc || volumeSource == a11yVolumeSrc) && 
                                 (voiceVolumeIndex == 0);
        ALOGV_IF(exemptFromCapping, "%s Custom volume source %d at vol=%f not capped", __func__,
                 volumeSource, volumeDb);
        if (volumeDb > maxVoiceVolDb && !exemptFromCapping) {
            ALOGV("%s Custom volume source %d at vol=%f overridden by volume group %d at vol=%f", __func__,
                  volumeSource, volumeDb, callVolumeSrc, maxVoiceVolDb);
            volumeDb = maxVoiceVolDb;
        }
    }

    // Headset rules: Reduce attenuation and relax limits for louder output
    if (!Intersection(deviceTypes, 
        ...
        if (isStreamActive(AUDIO_STREAM_MUSIC, SONIFICATION_HEADSET_MUSIC_DELAY) || 
            ...
            // Reduce attenuation from -6 dB to -3 dB
            volumeDb += -3.0f; // SONIFICATION_HEADSET_VOLUME_FACTOR_DB changed to -3.0f
            ...
            float musicVolDb = computeVolume(musicCurves, musicVolumeSrc, 
                                            ...
                                            musicDevice);
            // Increase minimum volume threshold from -36 dB to -24 dB
            float minVolDb = (musicVolDb > -24.0f) ? musicVolDb : -24.0f;
            if (volumeDb > minVolDb) {
                volumeDb = minVolDb;
                ALOGV("Custom computeVolume limiting volume to %f musicVol %f", minVolDb, musicVolDb);
            }
            if (Volume::getDeviceForVolume(deviceTypes) != AUDIO_DEVICE_OUT_SPEAKER &&
                ...
                // Relax A2DP notification volume adjustment
                if (volumeDb > -96.0f && 
                    (musicVolDb - 12.0f > volumeDb)) { // Reduced from 24 dB diff to 12 dB
                    ALOGV("%s increasing Custom volume for volume source=%d device=%s from %f to %f",
                          __func__, volumeSource, dumpDeviceTypes(deviceTypes).c_str(), volumeDb,
                          musicVolDb - 12.0f);
                    volumeDb = musicVolDb - 12.0f;
                }
            }
            ...
            volumeDb += -3.0f; // Reduce attenuation from -6 dB to -3 dB
        }
    }
                                           ...
                                           VolumeSource toVolumeSource)
{
    ...
    // Cache curve bounds
    const float minSrc = static_cast<float>(srcCurves.getVolumeIndexMin());
    const float maxSrc = static_cast<float>(srcCurves.getVolumeIndexMax());
    const float minDst = static_cast<float>(dstCurves.getVolumeIndexMin());
    const float maxDst = static_cast<float>(dstCurves.getVolumeIndexMax());

    // Validate curve bounds
    if (minSrc > maxSrc || minDst > maxDst) {
        ALOGE("%s: Invalid volume curve bounds: src [%f, %f], dst [%f, %f]",
              __func__, minSrc, maxSrc, minDst, maxDst);
        return srcIndex; // Return original index as fallback
    }

    // Clamp source index to valid range, preserving mute
    float clampedSrcIndex = srcIndex;
    if (srcIndex < minSrc) {
        clampedSrcIndex = (srcIndex == 0) ? 0.0f : minSrc;
    } else if (srcIndex > maxSrc) {
        clampedSrcIndex = maxSrc;
    }

    // Normalize source index to [0, 1] range
    float srcRange = maxSrc - minSrc;
    float normalizedSrc = (srcRange > 0) ? (clampedSrcIndex - minSrc) / srcRange : 0.0f;

    // Apply non-linear scaling (optional perceptual adjustment)
    float scaledValue = normalizedSrc; // Linear by default
    // Uncomment for logarithmic-like scaling:
    // scaledValue = (expf(normalizedSrc * logf(100.0f)) - 1.0f) / 99.0f;

    // Scale to destination range
    float dstRange = maxDst - minDst;
    float rescaled = minDst + (scaledValue * dstRange);

    // Round to nearest integer and clamp to destination bounds
    int result = static_cast<int>(roundf(rescaled));
    if (result < minDst) result = minDst;
    if (result > maxDst) result = maxDst;

    ALOGV("%s: Rescaling fromVolumeSource=%d, srcIndex=%d to toVolumeSource=%d, result=%d "
          "(minSrc=%f, maxSrc=%f, minDst=%f, maxDst=%f)",
          __func__, fromVolumeSource, srcIndex, toVolumeSource, result,
          minSrc, maxSrc, minDst, maxDst);
...
{//modifed code
    if (outputDesc == nullptr) {
        ALOGE("%s: Error!Custom Null output descriptor", __func__);
        return BAD_VALUE;
    }

    // If muted, don't change volume
    if (outputDesc->isMuted(volumeSource)) {
        ALOGD("%s:Custom VolumeSource %d is muted (mute count: %d, active: %d)", 
              __func__, volumeSource, outputDesc->getMuteCount(volumeSource), outputDesc->isActive(volumeSource));
        return NO_ERROR;
    }
    ...
    bool isVoiceCall = (callVolSrc == volumeSource);
    bool isBtSco = (btScoVolSrc == volumeSource);

    bool scoActive = isScoRequestedForComm();

    // Prevent conflicts between Bluetooth SCO and Voice Call volume
    if ((callVolSrc != btScoVolSrc) &&
        ((isVoiceCall && scoActive) || (isBtSco && !scoActive))) {
        ALOGV("%s: Skipping Custom volume change for %d (SCO Active: %d)", 
              __func__, volumeSource, scoActive);
        return NO_ERROR;
    }
    ...
    // Ensure fixed volume levels for Bluetooth SCO & VoIP over Bluetooth
    if (outputDesc->isFixedVolume(deviceTypes) ||
        (index != 0 && (isVoiceCall || isBtSco) &&
    ...
    ALOGD("%s: Set Custom volume %f dB for source %d on device %s (index %d)", 
          __func__, volumeDb, volumeSource, dumpDeviceTypes(deviceTypes).c_str(), index);

    // Special handling for Voice Call and Bluetooth SCO volume
    if (isVoiceCall || isBtSco) {
        float voiceVolume = isVoiceCall ? (float)index / (float)curves.getVolumeIndexMax()
                                        : (index == 0 ? 0.0f : 1.0f);

        if (voiceVolume != mLastVoiceVolume) {
            mpClientInterface->setVoiceVolume(voiceVolume, delayMs);
            ALOGD("%s: Set voice Custom volume: %f (last: %f)", __func__, voiceVolume, mLastVoiceVolume);
...
void AudioPolicyManager::applyStreamVolumes(const sp<AudioOutputDescriptor>& outputDesc,
                                            ...
                                            bool force)
{
    ALOGVV("custom applyStreamVolumes() for device %s", dumpDeviceTypes(deviceTypes).c_str());
...
{//modified code
    if (outputDesc == nullptr) {
        ALOGE("%s:Custom Null output descriptor, cannot set mute!", __func__);
        return;
    }

    ALOGD("%s: Custom Muting strategy %d, mute=%d, output ID=%d, delayMs=%d", 
          __func__, strategy, on, outputDesc->getId(), delayMs);

    // Use unordered_set for efficient lookup and avoid duplicates
    std::unordered_set<VolumeSource> sourcesToMute;

    for (const auto& attributes : mEngine->getAllAttributesForProductStrategy(strategy)) {
        VolumeSource source = toVolumeSource(attributes);
        if (sourcesToMute.insert(source).second) { // Only insert if it's not already present
            ALOGV("%s: Adding Custom VolumeSource %d for muting", __func__, source);
        }
    }

    // Apply mute for each unique VolumeSource
    for (const auto& source : sourcesToMute) {
        setVolumeSourceMute(source, on, outputDesc, delayMs, deviceTypes);
    }

    ALOGD("%s: Completed Custom muting for strategy %d", __func__, strategy);
}
